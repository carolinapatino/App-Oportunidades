CREATE PROC ReporteDeOportunidadesXEstatusVend (@EstatusId INT, @UsuarioId INT, @VendedorId INT, @Fecha DATE)
AS 
BEGIN 
SELECT O.OportunidadId, O.FechaCreacion, O.FechaCierre, O.MontoPresupuesto, O.Objetivo, O.ObservacionDeCierre, A.OrigenNombre, E.NombreEstatus,
 ...
 WHERE  E.EstatusId = O.Estatus AND O.Origen = A.OrigenId AND P.NroOportunidad = O.OportunidadId AND E.EstatusId = @EstatusId AND (O.IdCreador = @UsuarioId AND O.IdVendedor = @VendedorId) 
