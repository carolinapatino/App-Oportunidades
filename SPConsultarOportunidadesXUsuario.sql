...
BEGIN 
 ...
 WHERE  E.EstatusId = 1 AND O.Origen = A.OrigenId AND P.NroOportunidad = O.OportunidadId AND (O.IdCreador=@IDUsuario OR O.IdVendedor=@IDVendedor) 
 UNION 
 ...
 O.CodigoCliente, O.NombreCliente, O.IdCreador, O.IdVendedor, P.Productos
 FROM OportunidadDeVenta O, Estatus E, Origen A, (SELECT STRING_AGG (ProductoId, ',') AS Productos, NroOportunidad FROM ProductoOportunidad GROUP BY NroOportunidad) AS P
 WHERE  E.EstatusId = 1 AND O.Origen = A.OrigenId AND P.NroOportunidad = O.OportunidadId AND O.OportunidadId IN (SELECT A.Oportunidad FROM Accion A Where A.AsignadaA = @IDUsuario)) AS Oport
