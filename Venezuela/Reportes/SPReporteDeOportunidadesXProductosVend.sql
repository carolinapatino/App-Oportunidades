create  PROC [dbo].[ReporteDeOportunidadesXProductosVend] (@ProductoId INT, @UsuarioId INT, @VendedorId INT, @Fecha DATE)
AS 
BEGIN 
...
FROM    OportunidadDeVenta O
 ...
 WHERE 
		...
		AND (O.IdCreador = @UsuarioId OR O.IdVendedor= @VendedorId)
